...
(function(factory){
	...
	AmsifySelect.prototype = {
        ...
        createHTML : function() {
			...
			*/
			if(this.isSearchable) {
				...
				var search                = '<input type="text" class="'+this.classes.search.substring(1)+'" placeholder="Pesquise aqui..."/>';
			...
			var clear                 = '<button type="button" class="'+this.classes.clear.substring(1)+' '+this.clearClass+'">Limpar</button>';
			this.selectors.clear      = $(clear).appendTo(this.selectors.operations);

			var close                 = '<button type="button" class="'+this.classes.close.substring(1)+' '+this.closeClass+'">Fechar</button>';
        ...
        createList : function() {
			...
			if(this.isSearchable) {
				listHTML += '<li class="'+_self.classes.noResult.substring(1)+'">Nenhuma opção correspondente</li>';
			}
			return listHTML;
        },
